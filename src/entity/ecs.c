...
void _ecs_register_internal(enum ECSComponent id, size_t component_size,
  ...
  struct ComponentList list = {.component_size = component_size,
                               .system = system};
  ...
  for (size_t i = 0; i <= ECSCOMPONENT_LAST; i++) {
    ...
    for (size_t j = 0; j < self->capacity; j++) {
      ...
      if (ECS_TAG(component) & ECS_TAG_USED) {
        f(component,
          (struct Entity){.id = self->ids[j], .index = j, .ecs = self});
      }
    }
  }
}
...
struct Entity ecs_new(struct ECS *self) {
  ...
  if (i == self->capacity) {
    ...
    for (size_t j = 0; j <= ECSCOMPONENT_LAST; j++) {
      ...
      list->components =
          realloc(list->components, self->capacity * ECSCL_ELEMENT_SIZE(list));

      // initialize new allocation
      memset(list->components + (old_capacity * ECSCL_ELEMENT_SIZE(list)), 0,
